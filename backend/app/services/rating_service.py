...
from app.services.entity_helpers import (
    fetch_entity_snapshots,
    is_entity_approved,
    resolve_rating_titles,
)
...
def _apply_entity_snapshots(rating: Rating) -> None:
    title, subtitle, _ = fetch_entity_snapshots(rating.entity_type, rating.entity_id)
    rating.entity_title = title
    rating.entity_subtitle = subtitle
    rating.entity_image_url = cover_art_url(rating.entity_type.value, rating.entity_id)
...
def upsert_rating(
    ...
    parsed_type = _parse_entity_type(entity_type)
    if not is_entity_approved(parsed_type, entity_id):
        raise RatingError("La entidad no existe en MusicBrainz")
    ...
    if existing:
        ...
        _apply_entity_snapshots(existing)
        db.flush()
    ...
    _apply_entity_snapshots(rating)
    db.add(rating)
    db.flush()
...
def get_user_diary(db: Session, user_id: UUID) -> list[RatingDetailResponse]:
    ...
    for rating in ratings:
        title, subtitle = resolve_rating_titles(rating)
        score = float(rating.rating) if isinstance(rating.rating, Decimal) else rating.rating
        entries.append(
