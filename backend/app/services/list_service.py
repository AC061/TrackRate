...
def get_list_items(
    ...
    for item in items:
        title, subtitle = resolve_list_item_titles(item)
        result.append(
            ListItemResponse(
...
def add_list_item(
    ...
    if not is_entity_approved(parsed_type, entity_id):
        raise ListError("La entidad no existe en MusicBrainz")
    ...
    title, subtitle, _ = fetch_entity_snapshots(parsed_type, entity_id)
    item = ListItem(
        list_id=list_id,
    ...
    return ListItemResponse(
        ...
        entity_subtitle=item.entity_subtitle,
    )
